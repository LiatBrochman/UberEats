...
const BasketContextProvider = ({children}) => {
    ...
    const totalPrice = basketDishes.reduce((sum, basketDishes) => sum + basketDishes.quantity * basketDishes.Dish.price, restaurant?.deliveryFee);
...
DataStore.query(BasketDish, (bd) => bd.basketID.eq(basket.id)).then(setBasketDishes);
    ...
    return (<BasketContext.Provider value={{addDishToBasket, setRestaurant, restaurant, basket, basketDishes, totalPrice}}>{children}</BasketContext.Provider>)
};
